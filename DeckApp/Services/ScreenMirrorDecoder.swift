...
nonisolated final class ScreenMirrorDecoder {
    ...
    func decode(_ wireFrame: ScreenStreamWireFrame) async -> CMSampleBuffer? {
        ...
        // `AVSampleBufferDisplayLayer` schedules display against its own control timebase,
        // which defaults to the device's host clock (time since boot) -- not wall-clock time.
        // The wire timestamp is the Windows Agent's Unix-epoch millisecond clock, a completely
        // different reference point; feeding it straight through would make every frame's
        // presentation time land somewhere in that timebase's distant past or future, and the
        // layer would simply never display it. This is a live view, not an edited timeline, so
        // frames are always shown as soon as they're decoded: stamp with the current host time.
