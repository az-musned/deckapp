...
Phase 5 landed: guarded PC boot with configurable queue timeouts, and command progress that distinguishes accepted wake requests from confirmed PC availability, via a priority-ordered wake workflow (a user-defined Apple Shortcut, then client-side Wake-on-LAN, then a Home Assistant switch entity as fallback) plus live reachability polling and PC shutdown through the Windows Control Agent.

Do not implement direct vendor APIs for new integrations by default. TV control and the PC's smart plug go through Home Assistant or Apple Shortcuts, not a direct vendor cloud API. Keep the working Companion integration private to the LAN or private VPN, and keep Windows input behind the authenticated Windows Control Agent.

Govee and Gree are the deliberate exceptions to that rule: DeckApp connects to both vendor cloud APIs directly from Swift, with no Home Assistant involvement. For Gree, this followed live protocol testing that proved the AC's local LAN protocol unreachable on this unit's firmware while the cloud path (login, state reads, and writes) worked end-to-end against the real account and device. PC power-on deliberately does not follow that precedent: instead of a direct Tuya Cloud integration, DeckApp runs a user-named Apple Shortcut (`shortcuts://run-shortcut`) that the user builds themselves to turn on their smart plug, keeping DeckApp itself out of a third vendor's cloud API; a Home Assistant switch entity remains a configurable fallback if no Shortcut is set. Live Climate state is confirmed only while the app is foregrounded; backgrounded state is shown as stale rather than presented as current, since iOS does not keep the cloud MQTT session alive in the background.
