...
Phase 5 landed: guarded PC boot with configurable queue timeouts, and command progress that distinguishes accepted wake requests from confirmed PC availability, via a priority-ordered wake workflow (smart plug, then client-side Wake-on-LAN, then a Home Assistant switch entity as fallback) plus live reachability polling and PC shutdown through the Windows Control Agent.

Do not implement direct vendor APIs for new integrations by default. TV control goes through Home Assistant. Keep the working Companion integration private to the LAN or private VPN, and keep Windows input behind the authenticated Windows Control Agent.

Govee, Gree, and the PC-wake smart plug are the deliberate exceptions to that rule: DeckApp connects to Govee's and Gree's vendor cloud APIs directly from Swift, and triggers the PC's smart plug directly via the Tuya IoT Platform, all with no Home Assistant involvement for those specific calls. For Gree, this followed live protocol testing that proved the AC's local LAN protocol unreachable on this unit's firmware while the cloud path (login, state reads, and writes) worked end-to-end against the real account and device. For the PC smart plug, direct Tuya Cloud calls avoid depending on a Home Assistant `switch.*` entity mapping just to close a relay; a Home Assistant switch entity remains a configurable fallback if direct Tuya credentials aren't set. Live Climate state is confirmed only while the app is foregrounded; backgrounded state is shown as stale rather than presented as current, since iOS does not keep the cloud MQTT session alive in the background.
